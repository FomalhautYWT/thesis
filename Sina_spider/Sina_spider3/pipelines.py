...
import pymongo
from items import InformationItem, TweetsItem, RelationshipsItem,CommentsItem,RepostsItem,KeywordsItem
import MySQLdb
...
class MongoDBPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if isinstance(item, RelationshipsItem):
            ...
            commentsItem = dict(item)
            try:
                self.Comments.insert(dict(item))
            except Exception:
                pass
        elif isinstance(item, RepostsItem):
            repostsItem = dict(item)
            try:
                self.Reposts.insert(dict(item))
            except Exception:
                pass
        elif isinstance(item, KeywordsItem):
            keywordsItem = dict(item)
            try:
                self.Keywords.insert(dict(item))
            except Exception:
                pass
        return item
